...
L'outil de base de l'optimisation est le benchmark : l’évaluation des performances d’un code, ou seulement d’une partie. Deux fonctions complémentaires sont couramment utilisées pour cela : `profvis::profvis()` et `microbenmark::microbenmark()`. Elles sont chacune associés à un paquet éponyme. 
...
Le format de fichier rmarkdown (ou 'quarto' dans sa forme plus moderne) est un type de fichier fusionnant la syntaxe markdown avec du code R.
